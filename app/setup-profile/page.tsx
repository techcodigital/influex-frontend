...
export default function ViewProfile() {
  ...
  const name       = isBrand ? (profile.companyName || profile.name) : profile.name;
  // ✅ FIXED: get ALL categories as array
        ...
        <div style={{ maxWidth:680, margin:"0 auto", padding:"0 16px 60px" }}>
          ...
          <div className="vp-fade vp-fade-1" style={{ marginBottom:20 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:8, flexWrap:"wrap" }}>
              ...
              {/* ✅ FIXED: show ALL categories as separate tags */}
              {categories.map((cat, i) => (
                <span key={i} className="vp-tag" style={{ background:"#fef3c7", color:"#d97706" }}>
                  {capitalize(cat)}
                </span>
              ))}
              {profile.location && (
          ...
          <div className="vp-fade vp-fade-2" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(130px, 1fr))", gap:10, marginBottom:16 }}>
            ...
            {/* ✅ FIXED: show all categories in stats, not just first */}
            {categories.length > 0 && (
              <div className="vp-stat">
                <span className="vp-stat-val" style={{ fontSize: categories.length > 1 ? 13 : 20 }}>
          ...
          <div className="vp-fade vp-fade-3" style={{ background:"#fff", borderRadius:14, padding:"8px 20px", border:"1px solid #ebebeb", marginBottom:20 }}>
            ...
            {/* ✅ FIXED: platform shows actual clickable link */}
            {!isBrand && profile.platform && (
              <div className="vp-info-row">
                <div className="vp-icon">🔗</div>
                <div>
            ...
            {/* ✅ FIXED: show all categories in details */}
...
//                       onClick={() => setConfirmDelete(item.postId)}
//                     >
//                       {deletingId === item.postId ? "⏳" : "🗑"}
//                     </button>
//                   </div>
...
//   const categories = Array.isArray(profile.categories) ? profile.categories[0] : profile.categories;
...
//                 {capitalize(profile.role || "")}
//               </span>
//               {categories && <span className="vp-tag" style={{ background:"#fef3c7", color:"#d97706" }}>{capitalize(categories)}</span>}
...
//             )}
//             {categories && (
//               <div className="vp-stat"><span className="vp-stat-val">{capitalize(categories)}</span><span className="vp-stat-lbl">Category</span></div>
//             )}
//           </div>
...
//   // ── edit state ──
//   const [editPost, setEditPost]         = useState<PortfolioPost | null>(null);
...
//     setEditPost(post);
//     // detect type: if post has urls (videos) → video mode, else image mode
//     setEditType(post.urls && post.urls.length > 0 ? "video" : "image");
//     setEditFiles([]);
//     setEditPreviews([]);
//   };
...
//     if (!editPost || editFiles.length === 0) {
//       showToast("Please select at least one file.", "error"); return;
//     }
//     setEditLoading(true);
//     try {
//       const formData = new FormData();
//       // keep existing caption
//       formData.append("caption", editPost.caption || "");
//       // append new files under the "videos" key (backend uses upload.array("videos", 2))
//       editFiles.forEach(f => formData.append("videos", f));

//       const res = await fetch(`${API_BASE}/post/${editPost._id}`, {
//         method: "PUT",
//         headers: { Authorization: `Bearer ${getToken()}` },
//         body: formData,
//       });
...
//       // update local state with new urls/images from response
//       setPosts(prev => prev.map(p => {
//         if (p._id !== editPost._id) return p;
//         return {
//           ...p,
//           urls:   data.post?.urls   || data.urls   || p.urls,
//           images: data.post?.images || data.images || p.images,
//         };
//       }));
...
//         .pf-actions { position:absolute; top:9px; right:9px; display:flex; gap:5px; opacity:0; transition:opacity 0.2s; z-index:10; }
//         .pf-action-btn { width:28px; height:28px; border-radius:8px; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:13px; backdrop-filter:blur(6px); transition:all 0.15s; }
//         .pf-action-edit   { background:rgba(255,255,255,0.18); color:#fff; }
//         .pf-action-edit:hover { background:rgba(255,255,255,0.32); }
//         .pf-action-delete { background:rgba(239,68,68,0.75); color:#fff; }
//         .pf-action-delete:hover { background:rgba(239,68,68,1); }
//         .pf-action-btn:disabled { opacity:0.5; cursor:not-allowed; }
...
//         /* Confirm delete modal */
//         .pf-modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.55); z-index:9999; display:flex; align-items:center; justify-content:center; padding:20px; animation:pfFadeIn 0.2s ease; }
//         .pf-modal-box { background:#fff; border-radius:20px; padding:32px 28px 24px; max-width:360px; width:100%; text-align:center; animation:pfSlideUp 0.22s ease; position:relative; }
//         .pf-modal-icon { font-size:44px; margin-bottom:12px; }
//         .pf-modal-title { font-size:18px; font-weight:700; color:#111; margin-bottom:8px; font-family:'DM Sans',sans-serif; }
//         .pf-modal-sub { font-size:13px; color:#888; line-height:1.6; margin-bottom:22px; font-family:'DM Sans',sans-serif; }
//         .pf-modal-actions { display:flex; gap:10px; }
...
//         /* Edit modal */
//         .pf-edit-box { background:#fff; border-radius:20px; padding:28px; max-width:440px; width:100%; animation:pfSlideUp 0.22s ease; position:relative; max-height:90vh; overflow-y:auto; }
//         .pf-edit-close { position:absolute; top:14px; right:16px; background:#f5f5f3; border:none; width:30px; height:30px; border-radius:8px; font-size:14px; cursor:pointer; color:#888; display:flex; align-items:center; justify-content:center; }
//         .pf-edit-close:hover { background:#ebebeb; color:#333; }
//         .pf-edit-title { font-size:17px; font-weight:700; color:#111; margin-bottom:6px; font-family:'DM Sans',sans-serif; }
//         .pf-edit-subtitle { font-size:12px; color:#aaa; margin-bottom:20px; font-family:'DM Sans',sans-serif; }

//         /* Upload drop zone */
//         .pf-dropzone { border:2px dashed #d4d0f7; border-radius:14px; padding:28px 20px; text-align:center; cursor:pointer; transition:all 0.2s; background:#fafbff; }
//         .pf-dropzone:hover { border-color:#4f46e5; background:#f0f0ff; }
//         .pf-dropzone-icon { font-size:32px; margin-bottom:8px; }
//         .pf-dropzone-text { font-size:13px; color:#666; font-family:'DM Sans',sans-serif; margin-bottom:4px; }
//         .pf-dropzone-hint { font-size:11px; color:#bbb; font-family:'DM Sans',sans-serif; }

//         /* Preview grid */
//         .pf-preview-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(90px,1fr)); gap:8px; margin-top:14px; }
//         .pf-preview-item { position:relative; aspect-ratio:1; border-radius:10px; overflow:hidden; border:1.5px solid #ebebeb; }
//         .pf-preview-item img, .pf-preview-item video { width:100%; height:100%; object-fit:cover; }
//         .pf-preview-remove { position:absolute; top:4px; right:4px; width:20px; height:20px; border-radius:50%; background:rgba(239,68,68,0.85); border:none; color:#fff; font-size:10px; cursor:pointer; display:flex; align-items:center; justify-content:center; }

//         .pf-edit-actions { display:flex; gap:10px; margin-top:20px; }
//         .pf-edit-save { flex:1; padding:13px; border-radius:10px; border:none; background:linear-gradient(135deg,#4f46e5,#7c3aed); color:#fff; font-family:'DM Sans',sans-serif; font-size:14px; font-weight:700; cursor:pointer; transition:all 0.2s; box-shadow:0 2px 10px rgba(79,70,229,0.3); }
//         .pf-edit-save:hover:not(:disabled) { transform:translateY(-1px); }
//         .pf-edit-save:disabled { opacity:0.6; cursor:not-allowed; transform:none; }
...
//       {editPost && (
//         <div className="pf-modal-overlay" onClick={() => { setEditPost(null); setEditFiles([]); setEditPreviews([]); }}>
//           <div className="pf-edit-box" onClick={e => e.stopPropagation()}>
//             <button className="pf-edit-close" onClick={() => { setEditPost(null); setEditFiles([]); setEditPreviews([]); }}>✕</button>

//             <div className="pf-edit-title">
//               {editType === "video" ? "🎬 Replace Reel" : "📷 Replace Images"}
//             </div>
//             <div className="pf-edit-subtitle">
//               {editType === "video"
//                 ? "Upload a new video to replace the existing reel (max 2)"
//                 : "Upload new images to replace the existing post (max 3)"}
//             </div>

//             {/* Drop zone / file picker */}
//             <div className="pf-dropzone" onClick={() => fileInputRef.current?.click()}>
//               <div className="pf-dropzone-icon">{editType === "video" ? "🎬" : "📷"}</div>
//               <div className="pf-dropzone-text">
//                 {editFiles.length > 0 ? `${editFiles.length} file(s) selected` : "Click to select files"}
//               </div>
//               <div className="pf-dropzone-hint">
//                 {editType === "video" ? "MP4, MOV · Max 50MB each" : "JPG, PNG, WEBP · Max 10MB each"}
//               </div>
//             </div>
...
//                     }
//                     <button className="pf-preview-remove" onClick={() => {
...
//               <button className="pf-modal-cancel" onClick={() => { setEditPost(null); setEditFiles([]); setEditPreviews([]); }}>Cancel</button>
//               <button className="pf-edit-save" onClick={handleUpdate} disabled={editLoading || editFiles.length === 0}>
//                 {editLoading ? <><span className="pf-mini-spin" />Uploading...</> : "Upload & Save ✓"}
//               </button>
//             </div>
//           </div>
//         </div>
...
//             <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(130px, 1fr))", gap:10 }}>
//               {displayed.map((item, i) => {
//                 const post = posts.find(p => p._id === item.postId)!;
//                 return (
//                   <div key={item._id} className="pf-item" style={{ animationDelay:`${i*0.05}s` }}
//                     onClick={() => setLightbox({ type: item.type, src: item.src })}>
...
//                     <div className="pf-actions" onClick={e => e.stopPropagation()}>
//                       <button
//                         className="pf-action-btn pf-action-edit"
//                         title="Replace media"
...
//   // ── delete state ──
//   const [deletingId, setDeletingId]   = useState<string | null>(null);
//   const [confirmDelete, setConfirmDelete] = useState<string | null>(null); // postId to confirm
...
//       });
//       if (!res.ok) throw new Error("Update failed");
//       setPosts(prev => prev.map(p => p._id === editPost._id ? { ...p, caption: editCaption } : p));
//       showToast("Post updated successfully", "success");
//       setEditPost(null);
//     } catch {
//       showToast("Could not update post", "error");
...
//         /* ✅ Action buttons (edit + delete) — top right on hover */
//         .pf-actions { position:absolute; top:9px; right:9px; display:flex; gap:5px; opacity:0; transition:opacity 0.2s; z-index:10; }
//         .pf-action-btn { width:28px; height:28px; border-radius:8px; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; font-size:13px; backdrop-filter:blur(6px); transition:all 0.15s; }
//         .pf-action-edit   { background:rgba(255,255,255,0.18); color:#fff; }
//         .pf-action-edit:hover { background:rgba(255,255,255,0.32); }
...
//         /* Edit modal */
//         .pf-edit-box { background:#fff; border-radius:20px; padding:28px; max-width:400px; width:100%; animation:pfSlideUp 0.22s ease; }
//         .pf-edit-title { font-size:17px; font-weight:700; color:#111; margin-bottom:16px; font-family:'DM Sans',sans-serif; }
//         .pf-edit-label { font-size:11px; font-weight:600; color:#aaa; text-transform:uppercase; letter-spacing:0.07em; margin-bottom:6px; display:block; font-family:'DM Sans',sans-serif; }
//         .pf-edit-input { width:100%; padding:11px 14px; border:1.5px solid #ebebeb; border-radius:10px; font-size:14px; font-family:'DM Sans',sans-serif; color:#111; background:#fafafa; outline:none; resize:vertical; transition:border-color 0.2s; }
//         .pf-edit-input:focus { border-color:#4f46e5; background:#fff; }
//         .pf-edit-actions { display:flex; gap:10px; margin-top:18px; }
//         .pf-edit-save { flex:1; padding:12px; border-radius:10px; border:none; background:linear-gradient(135deg,#4f46e5,#7c3aed); color:#fff; font-family:'DM Sans',sans-serif; font-size:14px; font-weight:700; cursor:pointer; transition:all 0.2s; }
//         .pf-edit-save:hover:not(:disabled) { transform:translateY(-1px); }
//         .pf-edit-save:disabled { opacity:0.6; cursor:not-allowed; }
...
//             <div className="pf-modal-icon">🗑️</div>
//             <div className="pf-modal-title">Delete this post?</div>
//             <div className="pf-modal-sub">This action cannot be undone. The post will be permanently removed from your portfolio.</div>
//             <div className="pf-modal-actions">
//               <button className="pf-modal-cancel" onClick={() => setConfirmDelete(null)}>Cancel</button>
//               <button className="pf-modal-confirm" disabled={deletingId !== null} onClick={() => handleDelete(confirmDelete)}>
//                 {deletingId === confirmDelete ? <><span className="pf-mini-spin" />Deleting...</> : "Yes, Delete"}
...
//       {/* ── EDIT CAPTION MODAL ── */}
//       {editPost && (
//         <div className="pf-modal-overlay" onClick={() => setEditPost(null)}>
//           <div className="pf-edit-box" onClick={e => e.stopPropagation()}>
//             <div className="pf-edit-title">✏️ Edit Caption</div>
//             <label className="pf-edit-label">Caption</label>
//             <textarea
...
//             />
//             <div className="pf-edit-actions">
//               <button className="pf-modal-cancel" onClick={() => setEditPost(null)}>Cancel</button>
//               <button className="pf-edit-save" onClick={handleUpdate} disabled={editLoading}>
//                 {editLoading ? <><span className="pf-mini-spin" />Saving...</> : "Save Changes"}
//               </button>
//             </div>
//           </div>
//         </div>
...
//             <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(130px, 1fr))", gap:10 }}>
//               {displayed.map((item, i) => {
//                 // find the full post object for this item
//                 const post = posts.find(p => p._id === item.postId)!;
//                 return (
//                   <div key={item._id} className="pf-item" style={{ animationDelay:`${i*0.05}s` }}
//                     onClick={() => setLightbox({ type: item.type, src: item.src })}>
...
//                         className="pf-action-btn pf-action-edit"
//                         title="Edit caption"
//                         onClick={() => { setEditPost(post); setEditCaption(post.caption || ""); }}
//                       >
//                         ✏️
//                       </button>
//                       <button
...
//               <button className="vp-edit-btn vp-edit-desktop" onClick={() => router.push("/my-profile")}>✏️ Edit Profile</button>
//               {!isBrand && (
//                 <button className="vp-bank-btn vp-edit-desktop" onClick={() => router.push("/bank-details")}>🏦 Bank Details</button>
//               )}
//             </div>
//           </div>
