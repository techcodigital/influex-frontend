...
export default function DiscoveryPage() {
  ...
  const fetchAllCampaigns = async () => {
    try {
      ...
      merged.forEach((camp: any) => {
        ...
        if (Array.isArray(camp.subCategories)) {
          camp.subCategories.forEach((sc: string) => { if (sc?.trim()) subCatSet.add(sc.trim()); });
        } else if (typeof camp.subCategories === "string" && camp.subCategories?.trim()) {
          subCatSet.add(camp.subCategories.trim());
        }
      });
    ...
    return allCampaigns.filter((c) => {
      ...
      const subCatMatch = selectedSubCategory
        ? Array.isArray(c.subCategories)
          ? c.subCategories.some((sc: string) => sc.trim() === selectedSubCategory)
          : typeof c.subCategories === "string"
          ? c.subCategories.trim() === selectedSubCategory
          : false
        : true;
      const statusMatch = selectedStatus === "applied"
        ...
        ? !appliedIds.includes(c._id)
        : true;
      return cityMatch && catMatch && subCatMatch && statusMatch;
    });
  }, [allCampaigns, selectedCity, selectedCategory, selectedSubCategory, selectedStatus, appliedIds]);
  ...
  const hasFilter = selectedCity || selectedCategory || selectedSubCategory || selectedStatus;
  ...
  flex-direction: column;}
        ...
        /* Categories + SubCategories chips row */
        .disc-tags-row{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:16px;align-items:center}
      ...
      <div className="disc-page">
        ...
        <div className="disc-filters">
          ...
          {/* Sub Category */}
          <select
            className={`disc-select ${selectedSubCategory ? "active" : ""}`}
          ...
          {hasFilter && (
            <button className="disc-clear" onClick={clearFilters}>✕ Clear</button>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="disc-empty">
            ...
            <h3 className="disc-empty-title">
              {hasFilter ? "No matches found" : "No campaigns yet"}
            </h3>
            <p className="disc-empty-sub">
              {hasFilter ? "Try different filters or clear to see all" : "Check back soon!"}
            </p>
          </div>
        ) : (
          <div className="disc-grid">
            {filteredCampaigns.map((c, idx) => {
              ...
              const subCats: string[] = Array.isArray(c.subCategories)
                ? c.subCategories.filter(Boolean)
                : typeof c.subCategories === "string" && c.subCategories.trim()
                ? [c.subCategories.trim()]
                : [];
              ...
              return (
                ...
                >
                  ...
                  {/* Categories + Sub Categories in one row */}
                  {((c.categories?.length > 0) || subCats.length > 0) && (
                    <div className="disc-tags-row">
                      {c.categories?.length > 0 && c.categories.map((cat: string, i: number) => (
                        <span key={`cat-${i}`} className="disc-cat">{cat}</span>
                      ))}
                      {c.categories?.length > 0 && subCats.length > 0 && (
                        <div className="disc-tag-divider" />
                      )}
                      {subCats.map((sc: string, i: number) => (
                        <span key={`sc-${i}`} className="disc-subcat">{sc}</span>
                      ))}
                    </div>
                  )}
