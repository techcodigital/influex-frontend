...
//       // ✅ Save token from signup so verify-otp can use it
//       if (data.token) {
//         localStorage.setItem("token", data.token);
//         if (data.user) {
//           localStorage.setItem("cb_user", JSON.stringify({
...
// /* ─── OTP Step Component ─────────────────────────────────── */
// function OtpStep({ email, onBack, onSuccess }: { email: string; onBack: () => void; onSuccess: () => void }) {
...
//     try {
//       setLoading(true);
//       const res  = await fetch(`${API}/auth/verify-email?otp=${encodeURIComponent(otp)}`);
//       const data = await res.json();
...
//   return (
//     <div style={{minHeight:"85vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"48px 24px",background:"#ffffff"}}>
//       <div style={{maxWidth:440,width:"100%",background:"#fff",borderRadius:32,padding:"48px 40px",boxShadow:"0 20px 60px rgba(79,70,229,0.1)",border:"1.5px solid #e2e8f0",textAlign:"center"}}>

//         {verified ? (
//           <>
//             <div style={{width:72,height:72,background:"#f0fdf4",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 20px",fontSize:34}}>✅</div>
//             <h2 style={{fontSize:24,fontWeight:900,color:"#0f172a",marginBottom:8}}>Email Verified!</h2>
//             <p style={{fontSize:14,color:"#64748b"}}>Redirecting to login…</p>
//           </>
//         ) : (
//           <>
//             <div style={{width:72,height:72,background:"linear-gradient(135deg,#eef2ff,#ede9fe)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 24px",fontSize:34}}>
//               📧
//             </div>

//             <h2 style={{fontSize:26,fontWeight:900,color:"#0f172a",marginBottom:10,letterSpacing:"-0.02em"}}>
//               Verify Your Email
//             </h2>
//             <p style={{fontSize:14,color:"#64748b",marginBottom:4}}>We sent an OTP to</p>
//             <p style={{fontSize:15,fontWeight:700,color:"#4f46e5",marginBottom:28,wordBreak:"break-all"}}>{email}</p>

//             {error && (
//               <div style={{padding:"12px 16px",background:"#fef2f2",color:"#dc2626",borderRadius:12,fontSize:13,fontWeight:600,marginBottom:20}}>
//                 {error}
//               </div>
//             )}
...
//                   border:"1.5px solid #e2e8f0", borderRadius:16, fontSize:22,
...
//                 type="submit"
//                 disabled={loading}
//                 style={{width:"100%",padding:"15px",background:loading?"#a5b4fc":"linear-gradient(135deg,#4f46e5,#7c3aed)",color:"#fff",borderRadius:16,fontWeight:700,fontSize:15,border:"none",cursor:loading?"not-allowed":"pointer",boxShadow:"0 4px 20px rgba(79,70,229,0.3)",fontFamily:"inherit"}}
//               >
...
//                 Wrong email?{" "}
//                 <button onClick={onBack} style={{background:"none",border:"none",color:"#4f46e5",fontWeight:700,cursor:"pointer",fontSize:13,padding:0,fontFamily:"inherit"}}>
//                   Go back
//                 </button>
//               </p>
//             </div>
...
//   // After successful signup — show verify-email screen
...
//       // Still save token + user in case backend auto-logs in after verify
...
//         const userData = { ...data.user, token, hasProfile };
...
//       <OtpStep
//         email={signedUpEmail}
//         onBack={() => { setStep(2); setSignedUpEmail(""); setError(""); }}
