...
function ApplyPageInner() {
  ...
  const subCats: string[] = Array.isArray(campaign.subCategories)
    ? campaign.subCategories.filter(Boolean)
    : typeof campaign.subCategories === "string" && campaign.subCategories.trim()
    ? [campaign.subCategories.trim()]
    : [];
    ...
    <>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .cd-tag-subcat{background:#f0fdf4;border-color:#bbf7d0;color:#15803d}
            ...
            <div className="cd-card">
              ...
              {campaign.categories?.length > 0 && (
                <>
                  ...
                  <div className="cd-tags" style={{ marginBottom: 14 }}>
              ...
              {/* Sub Categories — niche aayegi */}
              {subCats.length > 0 && (
                <>
                  <span className="cd-sec-label">Sub Categories</span>
                  <div className="cd-tags" style={{ marginBottom: campaign.roles?.length ? 14 : 0 }}>
                    {subCats.map((sc: string, i: number) => (
                      <span key={i} className="cd-tag cd-tag-subcat">{sc}</span>
                    ))}
                  </div>
                </>
              )}
